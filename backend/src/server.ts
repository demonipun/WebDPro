...
dotenv.config();
// process.env.MONGO_URI -> access the value defined in dotenv file

import express from "express";
import cors from "cors";
// import { sample_foods, sample_tags, sample_users } from "./data";
// import jwt from "jsonwebtoken"; // shifted
import foodRouter from './routers/food.router'
import userRouter from "./routers/user.router";
import { dbConnect } from './configs/database.config';
dbConnect(); // configs to connect to the Db imported and used -> we are connected to Atlas MongoDb using Mongoose
...
// we have APIs for two models -> foods AND users
// creating APIs we will move APIs related to food to foodRouter
// and APIs related to users to userRouter

app.use("/api/foods", foodRouter) // anything we write in foodRouter is after the "/api/foods" so we need to remove it.
